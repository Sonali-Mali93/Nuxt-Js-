<template>
    <div class="mt-5">
        ...
        <table class="border border-black p-x-4">
            ...
            <tr v-for="per in allUser" :key="per">
                <td>{{ per.firstName }}</td>
                <td>{{ per.lastName }}</td>
                <td>{{ per.email }}</td>
                <td>{{ per.contact }}</td>
                <td><button class="border border-gray-400 px-2 py-1 max-w-xs rounded bg-teal-600 text-white"
                        type="submit" value="Submit">Update</button></td>
                <td v-for="per in allUser" :key="per"><button @click="deleteRow(per, allUser)"
                        class="border border-gray-400 px-3 py-2 max-w-xs rounded bg-teal-600 text-white" type="submit"
...
export default {
    ...
    methods: {
        ...
        deleteRow(index, user) {
            var idx = this.allUser.indexOf(user);
